...
/** DMA Engine: transfers N rows between HBM (off-chip) and UB (on-chip).
  *
  * DMA_LOAD  (HBM → UB): for each row, issue HBM read → wait for response → write to UB
  * DMA_STORE (UB → HBM): for each row, read from UB → write to HBM (single-cycle write)
  *
...
class DmaEngine(
  ...
  val sIdle :: sLoadReq :: sLoadWait :: sLoadWb :: sStoreRd :: sStoreRdWait :: sStoreWr :: sDone :: Nil = Enum(8)
