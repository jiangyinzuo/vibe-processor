  ...
  describe("双调度器性能测试") {
    ...
    it("纯计算程序（无内存访问）- 展示最佳并行度") {
      simulate(new ToyGpuTop(numSMs = 1, gmemLatency = 1)) { dut =>
        ...
        println("\n理论分析：")
        println("  - 4 个 Warp × 11 条指令 = 44 条指令")
        println("  - 单调度器：44 周期（串行）")
        println("  - 双调度器：22 周期（理想并行）")
        println(f"  - 实际周期：$totalCycles")
        println(f"  - 性能提升：${44.0 / totalCycles}%.2f×")
      }
    }
    ...
    it("内存密集程序（latency=10）- 展示延迟隐藏") {
      simulate(new ToyGpuTop(numSMs = 1, gmemLatency = 10)) { dut =>
        ...
        println(f"  - 实际周期：$totalCycles")
        println(f"  - 性能提升：${84.0 / totalCycles}%.2f×")
        println(f"  - 延迟隐藏效果：${21.0 / (totalCycles / 4.0)}%.2f×")
      }
    }
    ...
    it("混合程序（计算 + 内存）- 展示资源仲裁") {
      simulate(new ToyGpuTop(numSMs = 1, gmemLatency = 5)) { dut =>
        ...
        println(f"  - 实际周期：$totalCycles")
        println(f"  - 性能提升：${60.0 / totalCycles}%.2f×")
      }
    }
    ...
    it("性能对比总结") {
      ...
      println("\n性能提升：")
      println("  - 纯计算程序：接近 2× 吞吐量（理想情况）")
      println("  - 内存密集程序：通过 Warp 切换隐藏延迟，提升 1.5-2×")
      println("  - 混合程序：综合效果，提升 1.5-1.8×")
      println("\n与真实 GPU 的差距：")
