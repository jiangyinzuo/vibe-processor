...
- GPU 单 SM 内存密集用例在 `gmemLatency=10` 下有 187 个 stalled warp-cycle 和 11 个 no-eligible cycles；4-SM VADD 还会受到共享 4-stack HBM 子系统的 stack 交织、bank/row 时序和排队影响。
...
- `hbm_architecture.md`：真实 HBM 的 stack/channel/bank/row 层次和 HBM 子系统职责。
