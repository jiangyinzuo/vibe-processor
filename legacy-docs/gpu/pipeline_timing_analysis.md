...
`SCC` 是 `Strongly Connected Component`，即强连通分量。放在 RTL/Yosys
分析语境里，它通常用来检查网表中是否存在**组合反馈环**。

可以把网表看成一个有向图：

- wire / signal / cell output 是图里的节点。
- 组合逻辑依赖关系是节点之间的有向边。
- 如果一组节点能沿着组合边互相回到彼此，就形成了一个 SCC。

在同步数字电路里，正常的反馈应该被寄存器、锁存器或存储器时序边界隔开。例如：

```text
reg Q -> combinational logic -> reg D
```
...
```text
wire A -> combinational logic -> wire B -> combinational logic -> wire A
```
...
- 仿真中出现不稳定、振荡或 `X` 传播。
- 综合工具难以优化或给出不可预期结果。
- STA 无法把路径归类为正常的寄存器到寄存器路径。

因此 `scc` 检查的目标不是找最长路径，而是先确认设计没有明显的组合环问题。
本文中的：
...
表示 Yosys 没有在当前展开后的 GPU `SM` 层级里发现组合反馈环。它不能说明时序足够好，
但能说明后续 `ltp` 和 STA 分析没有被组合环这种更基础的问题污染。
